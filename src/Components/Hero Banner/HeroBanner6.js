...
function HeroBanner6() {
  ...
  return (
    ...
    <OwlCarousel className="banner-slider owl-carousel no-pb owl-2" {...owlOptions}>
      {bannerItems.map((item, index) => (
        <div key={index} className="item" style={{ backgroundImage: item.backgroundImage }}>
            ...
            <div className="row h-100 align-items-center text-center">
              <div className="col-lg-6 col-md-12 custom-py-1 position-relative z-index-1">
                ...
                <div className="animated3">
                  <a className="btn btn-primary btn-animated" href="#">{item.buttonLabel}</a>
                </div>
              </div>
            </div>
          </div>
